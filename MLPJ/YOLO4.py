...
def calculate_single_object_motor_output(x1, x2):
    ...
    left_motor = max_motor_output
    right_motor = max_motor_output

    if normalized_diff > 0: # 객체가 중앙보다 오른쪽에 있음 (오른쪽으로 회전)
        # 왼쪽 모터는 최대 출력 유지
        left_motor = max_motor_output
        # 오른쪽 모터는 255(중앙) -> 150(오른쪽 끝)으로 감소
        # normalized_diff가 0에서 1.0으로 갈수록 255에서 150으로 선형 감소
        # (1 - normalized_diff) * (max_motor_output - min_motor_output) + min_motor_output
        right_motor = int((1.0 - normalized_diff) * (max_motor_output - min_motor_output) + min_motor_output)
    elif normalized_diff < 0: # 객체가 중앙보다 왼쪽에 있음 (왼쪽으로 회전)
        # 오른쪽 모터는 최대 출력 유지
        right_motor = max_motor_output
        # 왼쪽 모터는 255(중앙) -> 150(왼쪽 끝)으로 감소
        # abs(normalized_diff)가 0에서 1.0으로 갈수록 255에서 150으로 선형 감소
        left_motor = int((1.0 - abs(normalized_diff)) * (max_motor_output - min_motor_output) + min_motor_output)
    else: # 객체가 중앙에 정확히 있음
        left_motor = max_motor_output
        right_motor = max_motor_output
    
    # 출력 범위 강제 (불필요해 보일 수 있으나 안전을 위해 유지)
